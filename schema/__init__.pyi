...
ST2 = TypeVar('ST2', bound=SchemaInput)
...
class And(Generic[ST]):
    ...
    def validate(self, data: ST2) -> Any: ...

class Or(And[ST], Generic[ST]):
    only_one: bool = ...
    match_count: int = ...
...
@overload
def _priority(s: Dict[Any, Any]) -> _Literal[4]: ...
@overload
...
@overload
def _priority(s: Callable[[T], RT]) -> Union[_Literal[1], _Literal[3]]: ...
@overload
...
class Optional(Schema[ST], Generic[ST]):
    default: Any = ...
    key: str = ...
...
class Hook(Schema[ST], Generic[ST]):
    key: ST = ...

    def __init__(self, *args: ST, **kwargs: Any) -> None: ...

class Forbidden(Hook[ST], Generic[ST]):
    def __init__(self, *args: ST, **kwargs: Any) -> None: ...
